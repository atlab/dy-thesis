...
\begin{equation}\label{eq:sample}
    ...
    \end{equation}
where $n$ is the number of time bins in $T$. The sample covariance matrix yields an unbiased estimate of the covariance matrix ($\E{C_{\sf sample}}=\Sigma$) if the mean, $\bar x$, is independent of the data sample, $\{x(t): t\in T$\}. When the mean is estimated from the same sample, $C_{\sf sample}$ becomes biased toward zero.  However, in all the cases where unbiasedness is required, we will estimate the mean from a separate sample.
